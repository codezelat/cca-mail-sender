...
class SchedulerService:
            ...
            try:
                with Session(engine) as session:
                    ...
                    for user in users_with_settings:
                        ...
                        logger.info(f"Processing contact {contact.email} for user {user.email}")
                        
                        # Initialize Service with User's Key
                        brevo = BrevoService(settings.brevo_api_key, settings.sender_email, settings.sender_name or "Sender")
                        
                        # EXECUTE SENDING LOGIC (Same as before)
                        # ...
                        # To avoid huge indentation drift, I will call a helper or inline it cleanly.
                        self._process_single_contact(session, brevo, contact, settings)
    ...
    def _process_single_contact(self, session, brevo, contact, settings):
        try:
            ...
            if not success:
                logger.error(f"Failed to create contact {contact.email}: {error}")
            ...
            success, msg_id = brevo.send_email(contact.email, contact.name, settings.subject or "Hello", html_content)
            if not success:
                logger.error(f"Failed to send email to {contact.email}: {msg_id}")
                contact.status = "failed"
            ...
            for i in range(max_retries):
                ...
                if status_data:
                    ...
                    if "bounced" in event_names or "error" in event_names or "soft_bounce" in event_names:
                        logger.error(f"Email bounced/failed for {contact.email}")
