...
async def upload_contacts(
    ...
    for _, row in df.iterrows():
        ...
        exists = session.exec(select(Contact).where(
            Contact.email == email).where(Contact.user_id == user.id)).first()
        if not exists:
...
async def get_stats(user: User = Depends(get_current_user), session: Session = Depends(get_session)):
    ...
    pending = session.query(Contact).filter(
        Contact.user_id == user.id, Contact.status == "pending").count()
    processing = session.query(Contact).filter(
        Contact.user_id == user.id, Contact.status == "processing").count()
    sent = session.query(Contact).filter(
        Contact.user_id == user.id, Contact.status == "sent").count()
    failed = session.query(Contact).filter(
        Contact.user_id == user.id, Contact.status == "failed").count()
...
async def get_activity(
    ...
    if sort_by == "status_priority":
        # Priority order: processing=0, pending=1, sent=2, failed=3
        # Use SQL CASE to order by status priority, then by updated_at DESC
        status_order = case(
            (Contact.status == "processing", 0),
            (Contact.status == "pending", 1),
            (Contact.status == "sent", 2),
            (Contact.status == "failed", 3),
            else_=4
        )
        contacts = session.exec(
            query.order_by(status_order, Contact.updated_at.desc())
            .offset((page - 1) * limit)
            .limit(limit)
        ).all()
    elif sort_by == "newest":
    ...
    else:  # "recent" - sort by updated_at
        contacts = session.exec(
...
async def update_contact(
    ...
    contact = session.exec(select(Contact).where(
        Contact.id == contact_id).where(Contact.user_id == user.id)).first()
    if not contact:
        raise HTTPException(status_code=404, detail="Contact not found")
